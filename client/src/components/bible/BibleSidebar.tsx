          ...
          <div className="flex gap-2 items-center">
            {viewMode === 'bookmarks' ? (
              <button
                ...
                className={`p-2 rounded-lg relative ${darkMode ? 'hover:bg-gray-700 text-amber-500' : 'hover:bg-amber-100 text-amber-600'}`}
                title="Volver a la Biblia"
              >
                <BookIcon size={20} />
              </button>
            ) : (
            ...
            <button
              ...
              className={`p-2 rounded-lg ${darkMode ? 'hover:bg-gray-700 text-amber-500' : 'hover:bg-amber-100 text-amber-600'}`}
              title="Cambiar tema"
            >
              {darkMode ? (
          ...
          {viewMode === 'reader' ? (
            <>
              <div className="mb-4">
                <select
                  ...
                  className={`w-full px-4 py-3 rounded-lg font-semibold text-lg ${darkMode
                        ...
                        {books
                          ...
                          .map(book => (
                            <button
                              ...
                              className={`w-full text-left px-3 py-2 rounded-lg mb-1 transition-colors ${selectedBook === book.id
            ...
            <>
              <div className="space-y-4">
                <section>
                  <h3 className={`text-sm font-semibold mb-2 px-2 uppercase tracking-wider ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    Filtros
                  </h3>
                  <div className="space-y-1">
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 rounded-lg transition-colors flex items-center gap-3
                                ${selectedMood === 'all'
                          ? (darkMode ? 'bg-amber-900/40 text-amber-500 font-medium' : 'bg-amber-100 text-amber-700 font-medium')
                          : (darkMode ? 'hover:bg-gray-700' : 'hover:bg-amber-50')
                        }`}
                    >
                      <LayoutList size={18} className={selectedMood === 'all' ? 'text-amber-500' : 'text-gray-400'} />
                ...
                <section>
                  <h3 className={`text-sm font-semibold mt-6 mb-2 px-2 uppercase tracking-wider ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                    Por Estado de Ánimo
                  </h3>
                  <div className="space-y-1">
                    {MOOD_COLORS.map((mood) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-lg transition-colors flex items-center gap-3
                            ...
                            ? (darkMode ? `${mood.bgDark} font-medium` : `${mood.bgLight} font-medium`)
                            : (darkMode ? 'hover:bg-gray-700' : 'hover:bg-amber-50')
                          }`}
                      >
                        <div className={`w-3 h-3 rounded-full ${darkMode ? mood.bgDark : mood.bgLight} border ${darkMode ? mood.borderDark : mood.borderLight}`} />
                        {mood.name}
                      </button>
                    ))}
                  </div>
